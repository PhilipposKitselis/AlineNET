// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GlobalAligner.cs" company="">
...
// </copyright>
// <summary>
//   Defines the GlobalAligner type.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace AlineNet.Aligners
{
    ...
    /// The global aligner.
    /// </summary>
    internal class GlobalAligner : Aligner
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="GlobalAligner"/> class.
        /// </summary>
        ...
        /// The parameters.
        /// </param>
        public GlobalAligner(Parameters parameters)
        {
        ...
        private void Alignment(Word wA, Word wB, int i, int j, int T, ref int getscore, char[] aback1, char[] aback2)
        {
            ...
            if (i == 0 && j == 0)
            {
                if (this.Allowed(this.Out) && !this.FallThru)
                {
                    ...
                    var score = T + (i + j) * Sigma.Skip(this.Parameters.SkipCost);
                    getscore = score / 100;
            ...
            {
                var subSc = Sigma.Sub(wA, i, wB, j, this.Parameters);
                ...
                var insSc = Sigma.Skip(this.Parameters.SkipCost);
                ...
                if(Parameters.Squashing)
                { 
                    var expSc = Sigma.Expand(wA, i, wB, j - 1, j, this.Parameters);
                ...
                var delSc = Sigma.Skip(this.Parameters.SkipCost);
                ...
                { 
                    var cmpSc = Sigma.Expand(wB, j, wA, i - 1, i, this.Parameters);
        ...
        /// The <see cref="AlignmentResult"/>.
        /// </returns>
        private AlignmentResult Align(Word wA, Word wB)
        {
            var aback1 = new char[50];
            var aback2 = new char[50];
            var score = 0;
            var lenA = (short) wA.PhoneticLength;
            var lenB = (short) wB.PhoneticLength;
            ...
            this.Similarity(wA, wB);

            this.DpScore = this.Score(lenA, lenB);
            ...
            for (short i = 0; i <= lenA; i++)
            {
                for (short j = 0; j <= lenB; j++)
                {
                    ...
                    if (this.ScoreMatrix[i][j] >= this.AcceptedScore)
                    {
                        ...
                        this.Out.Push(Constants.Lim, Constants.Lim);
                        this.Alignment(wA, wB, i, j, 0, ref score, aback1, aback2);
        ...
        /// The word b.
        /// </param>
        private void Similarity(Word wordA, Word wordB)
        {
            var lenA = wordA.PhoneticLength;
            var lenB = wordB.PhoneticLength;
            var sgmax = 0; // not meaningful for global case
            ...
            {
                this.ScoreMatrix[i][0] = this.ScoreMatrix[i - 1][0] + Sigma.Skip(this.Parameters.SkipCost);
            }

            for (short j = 1; j <= lenB; j++)
            {
                this.ScoreMatrix[0][j] = this.ScoreMatrix[0][j - 1] + Sigma.Skip(this.Parameters.SkipCost);
            }
            ...
            for (short i = 1; i <= lenA; i++)
            {
                for (short j = 1; j <= lenB; j++)
                {
                    ...
                    this.ScoreMatrix[i][j] = this.Max(m1, m2, m3, m4, m5);

                    if (this.ScoreMatrix[i][j] > sgmax)
                    {
                        sgmax = this.ScoreMatrix[i][j];
                    }
                }
            }
        }
